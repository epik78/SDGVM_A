...
! mean preserving interpolation Rymes & Myers 2001
    if (read_par) then
      call MEAN_PRES(year-yr0+1,mswrv,swrv)
...
! dsw((mnth-1)*30+day) = mswrv(year-yr0+1,mnth)
   daily((mnth-1)*30+day) = mswrv(iyear,mnth)
  enddo
enddo
...
do loop=1,iter
  ...
  do mnth=1,12
    sumdiff = 0.0d0
    do day=1,30
      sumdiff = sumdiff + (mswrv(iyear,mnth) - daily((mnth-1)*30+day))
    enddo
...
do mnth=1,12
  do day=1,30
    swrv(iyear,mnth,day) = daily((mnth-1)*30+day)                    
  enddo
enddo

function emu_runs(site_num)
    ...
    dirr{3} = '/home/sm1epk/SDGVM/SDGVM_ant/source/f03_ant';
    %Directory where the emu_param.dat is
    dirr{4} = '/home/sm1epk/SDGVM/SDGVM_ant/source/f03_ant';
    ...
    str{4} = 'emu_setup.dat';
    %Initial param file for emulators
    str{5} = 'emu_param.dat';
...
function setupfile(dirr,str,ii,jj,y,lat,lon,cov_flx)
    ...
    for kk=1:size(A,2)
        ...
        if(isempty(matches))
            ...
            %If the word params exists before '::' then write the directory
            %where the climate file is
            elseif(~isempty(strfind(C{1},'params')))
                fprintf(fid,[C{1},':: ',[dirr{2},str{1},num2str(ii),'/run_',num2str(jj)],'\n']);
